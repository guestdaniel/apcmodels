...
import numpy as np
from apcmodels.anf import calculate_verhulst2018_firing_rate


# def test_null_inputs():
#     """ Test to make sure that if model receives no input an error is raised """
#     try:
#         calculate_verhulst2018_firing_rate()
...
#     calculate_verhulst2018_firing_rate(_input=np.zeros(5000), cfs=np.array([500]))
#
#
# def test_spontaneous_firing_rate():
#     """ Test to make sure that if model receives empty short stimulus it has correct spont rate """
#     results = calculate_verhulst2018_firing_rate(_input=np.zeros(5000), cfs=np.array([1000]))
#     np.testing.assert_almost_equal(np.mean(results), 50, decimal=-1)  # check we're within 10 sp/s of 50 sp/s
#
#
