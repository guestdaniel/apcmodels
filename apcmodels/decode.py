...
def decode_staircase_procedure(ratefunc, rule, synthesizer, parameter, starting_value, step_sizes, transform,
    ...
    Implemented as a wrapper that can be applied to any ratefunc that accepts kwargs and returns a single firing rate
    simulation.
    ...
    Args:
        ...
        parameter (str): the name of a parameter to track in the adaptive procedure
        transform (function): a function that accepts the tracked value and params (below) and returns an increment
            in the original units of the variable that can be directly added.
    """
    def inner(params):
        """
        ...
        while n_trial < max_trials and len(reversals) < max_num_reversal:
            ...
            correct.append(one_step(params, value))
            # Handle logic of whether to change value or not
            if correct[-1] == 0:
                value = value + index_into_step_sizes(len(reversals))  # increase value
                direction.append(1)  # indicate we're going upward
            elif correct[-2:] == [1, 1]:
                value = value - index_into_step_sizes(len(reversals))  # decrease value
    ...
    def generate_stimuli(params, value):
        ...
        _input = synthesizer.synthesize(**params)
        # Synthesize initial target stimulus using copy of params
        params_copy = deepcopy(params)
        params_copy[parameter] = params_copy[parameter] + transform(value, params)
