...
def test_puretone_run_sequence():
    """ Check that puretone's synthesize_parameter_sequence() correctly accepts a list of dicts and returns a corresponding number of
    stimuli """
    synth = sy.PureTone()
    # Generate 1000 and 2000 Hz pure tones using synthesize_parameter_sequence
    results = synth.synthesize_parameter_sequence([{'freq': 1000, 'level': 50, 'phase': 0, 'dur': 1, 'fs': int(48e3), 'dur_ramp': 0.1},
...
def test_puretone_incremented_sequence():
    """ Check that puretone's synthesize_parameter_sequence() and create_incremented_sequence() successfully combine to produce two
    pure tones, one with a slightly higher frequency"""
    synth = sy.PureTone()
    # Generate 1000 and 2000 Hz pure tones using synthesize_parameter_sequence
    results = synth.synthesize_parameter_sequence(
        synth.create_incremented_parameter_sequence(baselines={'freq': 1000}, increments={'freq': 0.001}))
    # Generate references manually
    reference1 = sg.cosine_ramp(sg.scale_dbspl(sg.pure_tone(1000, 0, 1, int(48e3)), 50), 0.1, int(48e3))
